...
namespace Ugarit\Pail\Printers;

use Heritage\Support\Collection;
use Heritage\Support\Env;
use Heritage\Support\Str;
use Ugarit\Pail\Contracts\Printer;
use Ugarit\Pail\ValueObjects\MessageLogged;
use Ugarit\Pail\ValueObjects\Origin\Http;
use Ugarit\Pail\ValueObjects\Origin\Queue;
...
class CliPrinter implements Printer
{
    ...
    public function optionsHtml(MessageLogged $messageLogged): string
    {
        ...
        if ($origin instanceof Http) {
            ...
            $options = [
                $origin->command ? "scribe {$origin->command}" : null,
                $origin->queue,
                $origin->job,
            ];
        } else {
            $options = [
                $origin->command ? "scribe {$origin->command}" : 'scribe',
            ];
        }
